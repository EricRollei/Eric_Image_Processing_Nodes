...
class EDMSampler(Sampler):
    ...
    def convert_to_denoiser(
        ...
        def denoiser(x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
            ...
            cur_cfg_scale = self.get_cfg_scale(cfg_scale, c_noise[0].item())  # type: ignore[arg-type]
    ...
    def sample(
        ...
        if tiled:
            ...
            model.forward = make_tiled_fn(  # type: ignore[assignment]
        ...
        x = x_T * torch.sqrt(1.0 + self.sigmas[0] ** 2.0)  # type: ignore[index]
